...
class ActionEngine:
    ...
    def _evaluate_relationship_rule(
        ...
        if relationship_operator == RelationshipOperator.NOT_EXISTS:
            ...
            for edge in graph.edges:
                ...
                matching_sources.add(edge.source)
            ...
            for node_id, node in graph.nodes.items():
                ...
                if node_id in matching_sources:
                    continue
                ...
                results.append(
                    ActionResult(
                        ...
                        evidence=[
                            (
                                f"No {relationship.value} "
                                f"relationship to "
                                f"{related_node_type.value}"
                            )
                        ],
                        metadata={
                            "relationship": relationship.value,
        ...
        raise ValueError(
            f"Unsupported relationship operator: "
            f"{relationship_operator}"
        )
